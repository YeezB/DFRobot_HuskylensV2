...
bool husky_lens_protocol_receive(uint8_t data) {
    ...
    default:
        ...
        if (receive_index == receive_buffer[CONTENT_SIZE_INDEX] + CONTENT_INDEX) {
            ...
            receive_index = 0;
            DBG_PRINTLN("<--");
            DBG_HEX_ARRAY(receive_buffer, receive_buffer[CONTENT_SIZE_INDEX] + PROTOCOL_SIZE);
            DBG_PRINT("\n");
...
bool ProtocolV2::knock(void) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(ALGORITHM_ANY, COMMAND_KNOCK);
#ifdef LARGE_MEMORY
...
#ifdef LARGE_MEMORY
int16_t ProtocolV2::setMultiAlgorithm(eAlgorithm_t algo0, eAlgorithm_t algo1, eAlgorithm_t algo2,
                                      eAlgorithm_t algo3, eAlgorithm_t algo4) {
    DBG("\n");
    uint8_t *buffer       = husky_lens_protocol_write_begin(ALGORITHM_ANY, COMMAND_SET_MULTI_ALGORITHM);
    uint8_t  multiAlgoNum = 0;
    int16_t  algos[5]     = {ALGORITHM_ANY, ALGORITHM_ANY, ALGORITHM_ANY, ALGORITHM_ANY, ALGORITHM_ANY};
...
bool ProtocolV2::setMultiAlgorithmRatio(int8_t ratio0, int8_t ratio1, int8_t ratio2, int8_t ratio3, int8_t ratio4) {
    DBG("\n");
    uint8_t *buffer       = husky_lens_protocol_write_begin(ALGORITHM_MULTI, COMMAND_SET_MULTI_ALGORITHM_RATIO);
    uint8_t  multiAlgoNum = 0;
    int16_t  ratios[5];
    if (ratio0 != -1) {
        multiAlgoNum++;
...
bool ProtocolV2::takePhoto(eAlgorithm_t algo) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(algo, COMMAND_ACTION_TAKE_PHOTO);
    int      length = husky_lens_protocol_write_end();
...
bool ProtocolV2::takeScreenshot(eAlgorithm_t algo) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(algo, COMMAND_ACTION_TAKE_SCREENSHOT);
    int      length = husky_lens_protocol_write_end();
...
bool ProtocolV2::forgot(eAlgorithm_t algo) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(algo, COMMAND_ACTION_FORGOT);
    int      length = husky_lens_protocol_write_end();
...
bool ProtocolV2::drawRect(eAlgorithm_t algo, uint8_t colorID, int16_t x1, int16_t y1, int16_t x2, int16_t y2) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(algo, COMMAND_ACTION_DRAW_RECT);
...
bool ProtocolV2::clearRect(eAlgorithm_t algo) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(algo, COMMAND_ACTION_CLEAN_RECT);
    int      length = husky_lens_protocol_write_end();
...
bool ProtocolV2::drawText(eAlgorithm_t algo, uint8_t colorID, int16_t x, int16_t y, String text) {
    DBG("\n");
    uint8_t *buffer = husky_lens_protocol_write_begin(algo, COMMAND_ACTION_DRAW_TEXT);
    husky_lens_protocol_write_uint8(colorID);
    husky_lens_protocol_write_uint8(0);
